...
pub async fn create_vm(kvm_fd: RawFd, capacity: usize) {
    ...
    let mem = KvmUserspaceMemoryRegion {
        slot: 0 as __u32,
        flags: 0 as __u32,
        guest_phys_addr: ram_region as __u64,
        memory_size: capacity as __u64,
        userspace_addr: ram_region as __u64,
    };
    ...
    let ret = unsafe { set_memory_region(kvm_fd, std::ptr::addr_of!(mem)) };
    if ret.is_err() {
        println!("{ram_region:?}");
    }
}
...
*/
fn define_ram_region(capacity: usize) -> Result<*mut c_void, String> {
